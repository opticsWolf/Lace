...
|---|---|
| **`FramelessLaceMainWindow`** (`frameless_window.py`) | Frameless `QMainWindow` subclass; integrates the custom title bar into the `QMainWindow` layout (`setMenuWidget`) so the central widget sits below it, with an optional stacked menu bar (`menuBar()`). Accepts a `title_bar=` descriptor (`None` → `LaceStandardTitleBar`, a `QWidget` instance, a subclass, or a callable factory); `TitleBarDescriptor` / `_resolve_title_bar()` implement the resolution. Watches `QEvent.WinIdChange` and auto-heals the native chrome (`ensure_frameless_chrome()`), so GL children (`QWebEngineView`, `QOpenGLWidget`) cannot strip the DWM bits; `restore_frameless_chrome()` is the manual hatch. |
| **`FramelessLaceWindow`** (`frameless_window.py`) | Frameless top-level `QWidget` base for floating dock containers. Accepts the same `title_bar=` descriptor as the main window (`None` upgrades the base default to `LaceStandardTitleBar`). Same `WinIdChange` auto-heal; the heal also re-applies the float taskbar ex-style (`_apply_taskbar_presence`), which a recreated handle does not inherit. `FramelessFloatingDockContainer` reaches the heal via `super().event()`. |
| **`DockManager`** (`dock_manager.py`) | Exposes `main_title_bar` / `floating_title_bar` descriptors plus `create_main_title_bar(parent)` / `create_floating_title_bar(parent)` factories, so the main window and every floating container can use different custom title-bar classes. `main_title_bar` applies live via `setTitleBar` when the parent is frameless; `floating_title_bar` is consumed by `FramelessFloatingDockContainer` when no explicit title bar is passed. Installs both palette bridges itself (dock-tree target + app-wide target for top-level `QMenu`s, style untouched), so no manual `DockThemeBridge()` is needed. |
| **`LaceStandardTitleBar`** (`frameless_window.py`) | Shared custom-bar base. `canDrag()` vetoes drags from `QMenuBar`/`QMenu`/`QAbstractButton`/`QLineEdit` (`titlebar_blocks_drag()`); `paintEvent` fills the live theme background so QSS gaps cannot show; `insert_content_widget()` / `content_index_after_title()` anchor embeds after `titleLabel` instead of hardcoding indices. Double-click-to-maximize is **synchronous**. qframelesswindow's default handler posts an async `WM_SYSCOMMAND SC_MAXIMIZE/SC_RESTORE`, which Windows ignores while a mouse button is still held down — and a real double-click dispatches `MouseButtonDblClick` while the second click's button is still pressed — so the maximize silently failed (the "stale" double-click). Using `showMaximized()`/`showNormal()` directly takes effect regardless of the button state. Right-clicking the window icon opens the standard system menu (native `TrackPopupMenu` on Windows, `QMenu` fallback elsewhere); creating a Lace frameless window opts the process into system dark mode (`SetPreferredAppMode(AllowDark)`) so native menus follow the OS light/dark theme. Installed automatically on every frameless floating container; the frameless demo main window uses it too. |
