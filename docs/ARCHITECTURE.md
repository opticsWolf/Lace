...
| **`LaceStandardTitleBar`** (`frameless_window.py`) | `StandardTitleBar` whose double-click-to-maximize is **synchronous**. qframelesswindow's default handler posts an async `WM_SYSCOMMAND SC_MAXIMIZE/SC_RESTORE`, which Windows ignores while a mouse button is still held down — and a real double-click dispatches `MouseButtonDblClick` while the second click's button is still pressed — so the maximize silently failed (the "stale" double-click). Using `showMaximized()`/`showNormal()` directly takes effect regardless of the button state. Right-clicking the window icon opens the standard system menu (native `TrackPopupMenu` on Windows, `QMenu` fallback elsewhere). Installed automatically on every frameless floating container; the frameless demo main window uses it too. |
